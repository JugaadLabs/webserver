...
class monoDistance():
    ...
    def post_process(self, dets, image, vis_thresh=0.4, nms_iou_thresh=0.5, box_area_thresh=500):
        ...
        for j in self.cls_index_list:
            top_preds = dets[0, (dets[0, :, 5] == j), :6].astype(
                np.float32)  # find box in this class
            top_preds = top_preds[(top_preds[:, 4] > vis_thresh), :6].astype(
                np.float32)  # find box passing the threshold
            top_preds = top_preds[np.argsort(top_preds[:, 4])[
                ::-1], :]  # sort with confidence
            while top_preds.shape[0] > 0:  # non-maximum suppression
                box_max_conf = top_preds[0, :]
                box_area = (box_max_conf[2] - box_max_conf[0] + 1) * \
                    (box_max_conf[3] - box_max_conf[1] + 1)
    ...
    def add_det_label(self, img, bboxs, distances):
        for ind, bbox in enumerate(bboxs):
            ...
            txt = '{} {:.1f} ({:.1f}, {:.1f})m'.format(
                self.class_names[cat], conf, distances[ind][0], distances[ind][1])
    ...
    def blank_birds_view(self, img_size):
        ...
        def draw_half_circle_rounded(image, distance_factor, distance, boundary_color, boundary_thickness, horizontal_fov):
            ...
            cv2.ellipse(image, center, axes, angle, startAngle,
                        endAngle, boundary_color, boundary_thickness)

        img_size = int(img_size)
        birds_view_img = np.ones((img_size, img_size, 3), np.uint8)
        birds_view_img = birds_view_img * 255
        self.horizontal_fov = 75
        self.distance_factor = int(
            (img_size / 2) / np.sin(np.deg2rad(self.horizontal_fov / 2)) * 1.15 / 20)
        ...
        for dis_text in [5, 10, 15, 20]:
            textsize = cv2.getTextSize('{} m'.format(
                dis_text), self.font, img_size/1000, 1)[0]
            ...
            draw_half_circle_rounded(birds_view_img, self.distance_factor,
                                     dis_text, boundary_color, boundary_thickness, self.horizontal_fov)
    ...
    def birds_view(self, birds_view_img, selected_bboxs, bbox_distances):
        ...
        def add_object(image, size, center, color, txt):
            ...
            endAngle = 360
            cv2.ellipse(image, center, axes, angle, startAngle,
                        endAngle, color, thickness=1)
            cv2.putText(image, txt, center, self.font,
                        0.5, color, thickness=1, lineType=cv2.LINE_AA)

        for ind, bbox in enumerate(selected_bboxs):
            distance = bbox_distances[ind]
            horizontal_line = distance[1] * np.tan(np.deg2rad(
                self.horizontal_fov / 2)) * 2 * self.distance_factor
            size = (bbox[2] - bbox[0]) / \
                birds_view_img.shape[1] * horizontal_line
            center = distance * self.distance_factor
            center = (distance[0] * self.distance_factor + width /
                      2, height - distance[1] * self.distance_factor)
            ...
            txt = '{} {:.1f} ({:.1f}, {:.1f})m'.format(
                self.class_names[cat], conf, distance[0], distance[1])
    ...
    def detection_birdsview(self, img, vis_thresh, nms_iou_thresh, box_area_thresh):
        ...
        selected_bboxs = self.post_process(
            dets, img, vis_thresh, nms_iou_thresh, box_area_thresh)
...
def main():
    ...
    if save_bird_video:
        out_bird = cv2.VideoWriter(
            'output_bird.avi', format_avi, 10, (bird_view_size, bird_view_size))
    if save_hud_video:
        out = cv2.VideoWriter('output.avi', format_avi, 10, (input_w, input_h))
    if save_raw_video:
        out_raw = cv2.VideoWriter(
            'output_raw.avi', format_avi, 10, (input_w, input_h))

    image_size = (input_w, input_h)
    mono_distance = monoDistance(
        image_size, bird_view_size, trt_engine_path, class_names, cls_index_list)
        ...
        try:
            while True:
                ...
                img, birds_view_img, selected_bboxs, bbox_distances = \
                    mono_distance.detection_birdsview(
                ...
                print(' Latency = %.2f ms  (net=%.2f ms) FPS=%.2f' % (
                    (perf_counter()-t)*1000, (t3-t2)*1000, 1/((perf_counter()-t))))
                if save_hud_video:
                    out.write(img)
                if save_bird_video:
                    out_bird.write(birds_view_img)
